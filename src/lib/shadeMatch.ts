// Shade Match — DEW's acquisition wedge. Shade/undertone-aware picks for the makeup categories
// where color actually depends on your skin: foundation, concealer, blush, lip. It's
// content-based, so it works for a brand-new solo user on day one with no social graph — that's
// the point: Shade Match solves cold-start (works at n=1) while the Taste graph fills in. Where
// we DO have a graph, we layer in "fans with skin like yours" as honest social proof.
// Deterministic + explainable. PLACEHOLDER data — real shade SKUs would slot in later.
import { catalog, type Category, type Product, type Tone, type Undertone } from '../data/mockCatalog';
import { people, type Person } from '../data/social';
...
export function similarSkinPeople(tone?: Tone, undertone?: Undertone): Person[] {
  ...
  return people
    .map((p) => {
      const und = undertone && p.undertone !== undertone && undertone !== 'neutral' && p.undertone !== 'neutral';
